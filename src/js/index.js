...
function updateScrollbar() {
  ...
  // Calculate the height of the scrollbar thumb
  const thumbHeight =
    (containerHeight / contentHeight) * scrollbar.clientHeight;
  scrollbarThumb.style.height = `${thumbHeight}px`;
  ...
  scrollableContent.addEventListener("scroll", () => {
    ...
    const maxThumbPosition =
      scrollbar.clientHeight - scrollbarThumb.offsetHeight;
    const thumbPosition =
      (scrollTop / (contentHeight - containerHeight)) * maxThumbPosition;
    scrollbarThumb.style.transform = `translateY(${thumbPosition}px)`;
